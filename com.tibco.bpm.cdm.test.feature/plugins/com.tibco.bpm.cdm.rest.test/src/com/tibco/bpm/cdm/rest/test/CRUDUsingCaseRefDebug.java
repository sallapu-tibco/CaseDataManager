...
public class CRUDUsingCaseRefDebug extends Utils
{
	...
	private void assertErrorResponseVersionMismatch(String caseReference, String versionProvided, String actualVersion,
			Response response) throws IOException, DeploymentException, PersistenceException, InternalException,
			URISyntaxException, RuntimeApplicationException
	{
		...
		if (jsonPath.getString("errorCode").equalsIgnoreCase(properties.getCaseReferenceVersionMismatch().errorCode))
		{
		...
		else
		{
			...
			System.out.println("case ref passed - " + caseReference);
			System.out.println("versionProvided - " + versionProvided);
			...
			PersistenceException, InternalException, URISyntaxException, RuntimeApplicationException
	{
		...
		{
			caseRefs.clear();
			System.out.println("%%%%%%%%%%% Value in the loop " + i + "%%%%%%%%%%%%%%%%%%%");
			forceUndeploy(deploymentIdApp1);
			...
			System.out.println("%%%%%%%%%%% Actual case refernce is " + caseRefs.get(0) + "%%%%%%%%%%%%%%%%%%%");

			//update - version mismatch
			String tempString = alterCaseReference(caseRefs.get(0), "valid");
